...
"""Это задание к хакатону. При пересылке часть данных испортилась. 
Расшифруйте его, чтобы понять, что делать.

Соберите строку по полученной информации
Входные данные: текст как строка, IP как строка
Выходные данные: строка символов

Задание 1
Возможные пути дешифровки
1. Подозрительные повторяющиеся символы напомнинуют разделители
2. Кажется кто-то забыл сменить раскладку...
3. Ох, уж, этот копирайтер. Всё время путает русскую букву Е с английской

Сообщение:
_________________А___К!____________Х______
Это безумство и бардак! Хакеров не хватало на наших занятиях.\n
Вчера днём была совершена атака. Хакеры запустили вирус.\n
Теперь на главном сервере стоит пароль. Наша задача его узнать.
...
Выходные данные: строка-результат
...
Задание 4
Последняя утечка сообщила, что результат предыдущего задания - это не бессмыслица.
Нужно сместить каждый символ вправо на сумму цифр кодировки Windows


Входные данные: кодировка (только цифры) как строка символов
Выходные данные: пароль - строка символов
